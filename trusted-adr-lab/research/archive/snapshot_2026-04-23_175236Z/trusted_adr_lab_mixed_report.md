...
## Pooled detection
- Binary recall (conflict rows only): **50.0%**
- Core-N exact-type recall (semantic+constraint+temporal, `dependency_impact=n/a`): **50.0%**
- Pooled precision (typed TP / (TP + benign FP)): **57.1%**
- Benign specificity: **50.0%** (FP rate: 50.0%)
- Wrong-type alert rate on typed rows: **0.0%**

## Per-class exact-type recall
| class | injected | binary | exact | cross-type | abstained |
|-------|---------:|-------:|------:|-----------:|----------:|
| `semantic_contradiction` | 4 | 2 | 2 | 0 | 2 |
| `constraint_violation` | 2 | 0 | 0 | 0 | 2 |
| `temporal_invalidation` | 2 | 2 | 2 | 0 | 0 |
...
- `needs_review` precision (ambig→needs_review / (ambig+benign→needs_review)): **25.0%**
- `needs_review` recall on ambiguous rows: **25.0%**
- Unsafe forced-type rate on ambiguous rows: **0.0%**
- Outcome counts: `typed_conflict`=1, `needs_review`=9, `benign`=8, `unset`=0
